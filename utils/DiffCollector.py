...
class DiffCollector(object):
    ...
    def __init__(
        ...
        # set seed for action_space
        try:
            self._action_space.seed(self.env.config['seed'])
        except:
    ...
    def reset(
        ...
        self.data = Batch(
            ...
            first_order_gradient_last={},
            first_order_gradient_next={},
            second_order_gradient_next={},
        )
    ...
    def collect(
        ...
        if n_step is not None:
            ...
            raise TypeError(
                "Please specify at least one (either n_step or n_epissecond_order_diffode) "
        ...
        while True:
            # process data
            if (isinstance(self.data.obs_last, Batch)):
                self.data.update(first_order_gradient=self.data.obs)
            else:
                self.data.update(first_order_gradient=(self.data.obs-self.data.obs_last))
                
            if (isinstance(self.data.first_order_gradient_last, Batch)):
                self.data.update(second_order_gradient=self.data.first_order_gradient)
            else:
                self.data.update(second_order_gradient=(self.data.first_order_gradient-self.data.first_order_gradient_last))
                    
            self.data.update(obs_last=self.data.obs)
            self.data.update(first_order_gradient_last=self.data.first_order_gradient)
            
            # obs = [obs, first_order_gradient, second_order_gradient]
            # self.data.update(obs=[self.data.obs, self.data.first_order_gradient, self.data.second_order_gradient])
